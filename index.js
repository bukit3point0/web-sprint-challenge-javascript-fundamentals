...
function summation(addThem) {
  let result = 0;
  for (let i=0; i<addThem; i++) {
    result = result + i + 1;
  }
  return result;
}
...
const zooAnimals = [
  ...
  */
  ...
  function lowerCaseNames(array){
    const animalNames = array.map (aname => aname.animal_name);
    const lowerCase = animalNames.map (animal => animal.toLowerCase());
    return lowerCase;
  }
  ...
  */
  ...
  function USApop(array){
    const USApop = array.reduce((total, animal) => {
...
class CuboidMakerTwo{
  constructor(protos) {
    this.length = protos.length;
    this.width = protos.width;
    this.height = protos.height;
  }
  volume () {
    return this.length * this.width * this.height;
  }
